...
# Purpose: Check for memory leaks in custom DamOS programs.
# Usage: ./memcheck-full.sh
# Platform: Linux/Debian
# Script version: 0.0.2.2
# Date last edited: 04/04/2014
...
echo -e "${txtred}Memory leak test for Custom DamOS programs and utilities." ; tput sgr0
...
echo -e "If you need help with building the custom DamOS programs look for an automatic build script named build.sh. In the directory of the build script run the command: \"$ ./build.sh\" and the programs will be built for you."
 
echo

echo 'The following programs will be tested: ./reader ./loe ./logger ./processor ./server ./client'
...
echo 'Testing for memory leaks in custom DamOS programs using Valgrind...'
...
# alias leak-check='valgrind --leak-check=full --show-reachable=yes -v'

echo 'Testing for memory leaks in menu reader utility now...'
echo -e '\nMenu Reader Utility\n' >> $outputFile

echo 'Arguments passed: ./menu'
(valgrind --leak-check=full --show-reachable=yes -v ./menu) >> $outputFile 2>&1

echo 'Arguments passed: ./menu menufile.txt'
(valgrind --leak-check=full --show-reachable=yes -v ./menu menufile.txt) >> $outputFile 2>&1
...
# (valgrind --leak-check=full --show-reachable=yes -v ./loe) >> $outputFile 2>&1

echo -e "Arguments passed: ./loe -e inlineKey \"Test in-line encryption.\""
(valgrind --leak-check=full --show-reachable=yes -v ./loe -e inlineKey "Test in-line encryption.") >> $outputFile 2>&1

echo -e "Arguments passed: ./loe -d inlineKey \"Test in-line encryption.\""
(valgrind --leak-check=full --show-reachable=yes -v ./loe -d inlineKey "Test in-line encryption.") >> $outputFile 2>&1
...
# (valgrind --leak-check=full --show-reachable=yes -v ./loe -D inlineKey) >> $outputFile 2>&1

echo -e "Arguments passed: ./loe -e key.txt \"Test in-line encryption.\""
(valgrind --leak-check=full --show-reachable=yes -v ./loe -e key.txt "Test in-line encryption.") >> $outputFile 2>&1
...
(valgrind --leak-check=full --show-reachable=yes -v ./logger) >> $outputFile 2>&1

echo 'Arguments passed: ./logger /dev/input/event2'
(valgrind --leak-check=full --show-reachable=yes -v ./logger /dev/input/event2) >> $outputFile 2>&1

echo 'Arguments passed: ./logger /dev/input/event2 logFile.txt'
(valgrind --leak-check=full --show-reachable=yes -v ./logger /dev/input/event2 logFile.txt) >> $outputFile 2>&1
...
(valgrind --leak-check=full --show-reachable=yes -v ./processor) >> $outputFile 2>&1

echo 'Arguments passed: ./processor log.txt'
(valgrind --leak-check=full --show-reachable=yes -v ./processor log.txt) >> $outputFile 2>&1
...
(valgrind --leak-check=full --show-reachable=yes -v ./server) >> $outputFile 2>&1

echo 'Arguments passed: ./server 127.0.0.1'
(valgrind --leak-check=full --show-reachable=yes -v ./server 127.0.0.1) >> $outputFile 2>&1
...
(valgrind --leak-check=full --show-reachable=yes -v ./client) >> $outputFile 2>&1

echo 'Arguments passed: ./client 127.0.0.1'
(valgrind --leak-check=full --show-reachable=yes -v ./client 127.0.0.1) >> $outputFile 2>&1

echo 'Arguments passed: ./client 127.0.0.1 5555'
(valgrind --leak-check=full --show-reachable=yes -v ./client 127.0.0.1 5555) >> $outputFile 2>&1

# echo 'Arguments passed: ./client 127.0.0.1 5556 &'
# (valgrind --leak-check=full --show-reachable=yes -v ./client 127.0.0.1 5556 &) >> $outputFile 2>&1

echo 'Done testing for memory leaks in client program.'

echo

echo 'Done testing for memory leaks in custom DamOS programs and utilities.'
