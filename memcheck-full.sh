...
# alias leak-check='valgrind --leak-check=full --show-reachable=yes'

echo 'Testing for memory leaks in menu reader utility now...'
echo -e '\nMenu Reader Utility\n' >> $outputFile

echo 'Arguments passed: ./menu'
(valgrind --leak-check=full --show-reachable=yes ./menu) >> $outputFile 2>&1

echo 'Arguments passed: ./menu menufile.txt'
(valgrind --leak-check=full --show-reachable=yes ./menu menufile.txt) >> $outputFile 2>&1
...
# (valgrind --leak-check=full --show-reachable=yes ./loe) >> $outputFile 2>&1

echo -e "Arguments passed: ./loe -e inlineKey \"Test in-line encryption.\""
(valgrind --leak-check=full --show-reachable=yes ./loe -e inlineKey "Test in-line encryption.") >> $outputFile 2>&1

echo -e "Arguments passed: ./loe -d inlineKey \"Test in-line encryption.\""
(valgrind --leak-check=full --show-reachable=yes ./loe -d inlineKey "Test in-line encryption.") >> $outputFile 2>&1
...
# (valgrind --leak-check=full --show-reachable=yes ./loe -D inlineKey) >> $outputFile 2>&1

echo -e "Arguments passed: ./loe -e key.txt \"Test in-line encryption.\""
(valgrind --leak-check=full --show-reachable=yes ./loe -e key.txt "Test in-line encryption.") >> $outputFile 2>&1
...
(valgrind --leak-check=full --show-reachable=yes ./logger) >> $outputFile 2>&1

echo 'Arguments passed: ./logger /dev/input/event2'
(valgrind --leak-check=full --show-reachable=yes ./logger /dev/input/event2) >> $outputFile 2>&1

echo 'Arguments passed: ./logger /dev/input/event2 logFile.txt'
(valgrind --leak-check=full --show-reachable=yes ./logger /dev/input/event2 logFile.txt) >> $outputFile 2>&1
...
(valgrind --leak-check=full --show-reachable=yes ./processor) >> $outputFile 2>&1

echo 'Arguments passed: ./processor log.txt'
(valgrind --leak-check=full --show-reachable=yes ./processor log.txt) >> $outputFile 2>&1
...
(valgrind --leak-check=full --show-reachable=yes ./server) >> $outputFile 2>&1

echo 'Arguments passed: ./server 127.0.0.1'
(valgrind --leak-check=full --show-reachable=yes ./server 127.0.0.1) >> $outputFile 2>&1
...
(valgrind --leak-check=full --show-reachable=yes ./client) >> $outputFile 2>&1

echo 'Arguments passed: ./client 127.0.0.1'
(valgrind --leak-check=full --show-reachable=yes ./client 127.0.0.1) >> $outputFile 2>&1

echo 'Arguments passed: ./client 127.0.0.1 5555'
(valgrind --leak-check=full --show-reachable=yes ./client 127.0.0.1 5555) >> $outputFile 2>&1

# echo 'Arguments passed: ./client 127.0.0.1 5556 &'
# (valgrind --leak-check=full --show-reachable=yes ./client 127.0.0.1 5556 &) >> $outputFile 2>&1
